...
class Solution
{
    ...
    public ArrayList<ArrayList<Integer>> printPaths(Node root, int sum)
    {
        ...
        helperFun(root,sum,0,subans);
        return ans;
    }
    public void helperFun(Node root,int target,int sum,ArrayList<Integer> subans){
        if(root==null){
            return;
        }
        sum+=root.data;
        ...
        helperFun(root.left,target,sum,subans);
        helperFun(root.right,target,sum,subans);
